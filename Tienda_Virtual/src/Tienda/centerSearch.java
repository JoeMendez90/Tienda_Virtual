...
/*
    private void BuscarProductos() {
        cards.get(0).setLayout(null);
        cards.get(0).setBackground(Color.decode("#424242"));
        cards.get(0).setBounds(5, 110, 1265, 565);
    ...
    public void Actions(String set) {
        ...
        switch(subAction[0]){
            case "set":
                if(dip!=Integer.valueOf(subAction[1])){
                    System.out.println("acction");
                    dip=Integer.valueOf(subAction[1]);
                    box.setSelectedIndex(dip);
    ...
    public DinamicArray<Producto> search2(){
        ...
        if(dip==div){
            for (int i = dip*50; i < search.tam; i++) {
                search2.addBack(search.get(i));
            }
        }else{
            for (int i = dip*50; i < dip*50+50; i++) {
                search2.addBack(search.get(i));
            }
        }
        return search2;
    ...
    public JButton createProduct(Producto producto){
